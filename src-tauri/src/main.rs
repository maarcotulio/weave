...
#[derive(Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct CanvasEdge { id: String, canvas_id: String, source_node_id: String, target_node_id: String, relationship_id: String }
#[derive(Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
#[derive(Clone, Serialize)]
#[serde(rename_all = "camelCase")]
struct ProjectSnapshot { project: Project, stories: Vec<Story>, chapters: Vec<Chapter>, scene_sets: Vec<SceneSet>, scenes: Vec<Scene>, continuous_drafts: Vec<ContinuousDraft>, worldbuilding_items: Vec<WorldbuildingItem>, relationships: Vec<DomainRelationship>, document_links: Vec<DocumentLink>, markdown_notes: Vec<MarkdownNote>, note_links: Vec<NoteLink>, canvases: Vec<StoryCanvas>, style_profile: EditorStyleProfile, writing_stats: WritingStats, status: OperationStatus }
#[derive(Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
fn resolve_wiki_target(store: &Store, target_text: &str) -> Option<String> { let target = target_text.trim().to_lowercase(); let mut candidates: Vec<String> = store.worldbuilding_items.iter().filter(|item| item.title.to_lowercase() == target || item.aliases.iter().any(|alias| alias.to_lowercase() == target)).map(|item| item.id.clone()).collect(); candidates.extend(store.markdown_notes.iter().filter(|note| note.title.to_lowercase() == target).map(|note| note.id.clone())); (candidates.len() == 1).then(|| candidates.remove(0)) }
fn rebuild_note_links(store: &Store, note: &MarkdownNote, previous: &[NoteLink]) -> Vec<NoteLink> { parse_wiki_links(&note.markdown).into_iter().map(|parsed| { let existing = previous.iter().find(|link| link.target_text == parsed.target_text && link.label == parsed.label && link.occurrence == parsed.occurrence); let target_id = existing.and_then(|link| link.target_id.clone()).filter(|target_id| entity(store, target_id).is_ok()).or_else(|| resolve_wiki_target(store, &parsed.target_text)); NoteLink { id: existing.map(|link| link.id.clone()).unwrap_or_else(|| id("note-link")), note_id: note.id.clone(), target_id, target_text: parsed.target_text, label: parsed.label, start: parsed.start, end: parsed.end, occurrence: parsed.occurrence, created_at: existing.map(|link| link.created_at.clone()).unwrap_or_else(timestamp) } }).collect() }
...
#[tauri::command]
fn create_project(directory: String, name: String, app: State<'_, Mutex<AppState>>) -> Result<Project, String> { let mut state = app.lock().map_err(|_| "Project lock poisoned")?; let root = PathBuf::from(&directory); let _ = database_for(&root)?; state.root = Some(root); let project = Project { id: id("project"), name, directory, schema_version: 3, created_at: timestamp(), updated_at: timestamp() };  state.store = Store::default(); state.store.project = Some(project.clone()); status(&mut state, "saved", "Project created offline"); state.persist()?; Ok(project) }

#[tauri::command]
fn open_project(directory: String, app: State<'_, Mutex<AppState>>) -> Result<Project, String> { let mut state = app.lock().map_err(|_| "Project lock poisoned")?; let root = PathBuf::from(&directory); let db = database_for(&root)?; state.store = load_store(&db)?; state.root = Some(root); if let Some(project) = state.store.project.as_mut() { project.schema_version = project.schema_version.max(3); } let project = state.store.project.clone().ok_or_else(|| "Project metadata is missing".to_string())?; status(&mut state, "saved", "Project opened offline"); state.persist()?; Ok(project) }
#[tauri::command]
...
fn delete_worldbuilding_item(item_id: String, expected_revision: i64, mode: Option<String>, app: State<'_, Mutex<AppState>>) -> Result<(), String> {
    ...
    if mode.as_deref() == Some("remove-references") {
        ...
        state.store.canvas_edges.retain(|value| !relationships.contains(&value.relationship_id) && !nodes.contains(&value.source_node_id) && !nodes.contains(&value.target_node_id));
    }
...
#[tauri::command]
fn delete_relationship(relationship_id: String, app: State<'_, Mutex<AppState>>) -> Result<(), String> { let mut state = app.lock().map_err(|_| "Project lock poisoned")?; relationship(&state.store, &relationship_id)?; state.store.relationships.retain(|value| value.id != relationship_id); state.store.canvas_edges.retain(|edge| edge.relationship_id != relationship_id); status(&mut state, "saved", "Relationship removed from links and canvases"); state.persist()?; Ok(()) }
#[tauri::command]
...
#[tauri::command]
fn delete_markdown_note(note_id: String, expected_revision: i64, mode: Option<String>, app: State<'_, Mutex<AppState>>) -> Result<(), String> { let mut state = app.lock().map_err(|_| "Project lock poisoned")?; let note = markdown_note(&state.store, &note_id)?.clone(); if note.revision != expected_revision { status(&mut state, "revision-conflict", "Save stopped: this note changed elsewhere"); state.persist()?; return Err(format!("Revision conflict: expected {expected_revision}, current {}", note.revision)); } let relationships: Vec<String> = state.store.relationships.iter().filter(|value| value.source_id == note_id || value.target_id == note_id).map(|value| value.id.clone()).collect(); let incoming = state.store.note_links.iter().filter(|value| value.note_id != note_id && value.target_id.as_deref() == Some(&note_id)).count(); let nodes: Vec<String> = state.store.canvas_nodes.iter().filter(|value| value.entity_id == note_id).map(|value| value.id.clone()).collect(); if mode.as_deref().unwrap_or("reject") != "remove-references" && (!relationships.is_empty() || incoming > 0 || !nodes.is_empty()) { return Err(format!("Cannot delete {}: references remain. Choose remove-references to preserve unresolved Markdown links.", note.title)); } if mode.as_deref() == Some("remove-references") { state.store.relationships.retain(|value| !relationships.contains(&value.id)); state.store.note_links.retain(|value| value.note_id != note_id); state.store.note_links.iter_mut().filter(|value| value.target_id.as_deref() == Some(&note_id)).for_each(|value| value.target_id = None); state.store.canvas_nodes.retain(|value| !nodes.contains(&value.id)); state.store.canvas_edges.retain(|value| !relationships.contains(&value.relationship_id) && !nodes.contains(&value.source_node_id) && !nodes.contains(&value.target_node_id)); } else { state.store.note_links.retain(|value| value.note_id != note_id); } state.store.markdown_notes.retain(|value| value.id != note_id); status(&mut state, "saved", "Markdown note deleted safely"); state.persist()?; Ok(()) }
#[tauri::command]
...
#[tauri::command]
fn repair_note_link(link_id: String, target_id: String, app: State<'_, Mutex<AppState>>) -> Result<NoteLink, String> { let mut state = app.lock().map_err(|_| "Project lock poisoned")?; entity(&state.store, &target_id)?; let link = state.store.note_links.iter_mut().find(|link| link.id == link_id).ok_or_else(|| "Unknown Markdown link".to_string())?; link.target_id = Some(target_id); let value = link.clone(); status(&mut state, "saved", "Markdown link repaired by stable ID"); state.persist()?; Ok(value) }
...
#[tauri::command]
fn recover_from_backup(backup_id: String, app: State<'_, Mutex<AppState>>) -> Result<OperationStatus, String> { let mut state = app.lock().map_err(|_| "Project lock poisoned")?; let connection = Connection::open(state.db_path()?).map_err(|e| e.to_string())?; let json: String = connection.query_row("SELECT state_json FROM backups WHERE id=?1", params![backup_id], |row| row.get(0)).map_err(|e| e.to_string())?; state.store = serde_json::from_str(&json).map_err(|e| e.to_string())?; if let Some(project) = state.store.project.as_mut() { project.schema_version = project.schema_version.max(3); } status(&mut state, "recovered", "Recovered backup; verify the project before editing"); state.persist()?; Ok(state.store.status.clone()) }
#[tauri::command]
...
pub fn run() { tauri::Builder::default().manage(Mutex::new(AppState::default())).invoke_handler(tauri::generate_handler![create_project, open_project, get_project, create_story, create_chapter, create_scene, list_stories, list_chapters, list_scene_sets, list_scenes, rename_scene, reorder_scene, create_worldbuilding_item, update_worldbuilding_item, delete_worldbuilding_item, list_worldbuilding_items, search_worldbuilding, create_relationship, delete_relationship, list_relationships, list_backlinks, create_document_link, repair_document_link, list_document_links, create_markdown_note, update_markdown_note, delete_markdown_note, list_markdown_notes, search_markdown_notes, list_note_links, repair_note_link, create_canvas, list_canvases, canvas_projection, add_canvas_node, remove_canvas_node, connect_canvas_nodes, save_canvas_layout, get_document, get_revision, save_document, get_style_profile, update_style_profile, writing_stats, set_daily_word_target, enter_continuous_draft, get_continuous_draft, keep_continuous_separate, automatically_split_continuous, compose_chapter, integrity_check, create_backup, recover_from_backup, get_status, project_snapshot, write_export]).run(tauri::generate_context!()).expect("error while running Weave"); }
