...
describe('Manuscript header chapter control', () => {
  it('creates the next chapter in the selected story rather than a project', () => {
    ...
    expect(app).toContain("const addChapter = () => void run(async () => { if (!selectedStoryId) return; await autosave.flush(); const chapterNumber = (snapshot?.chapters.filter((item) => item.storyId === selectedStoryId).length ?? 0) + 1; await repository.createChapter(selectedStoryId, `Chapter ${chapterNumber}`); await refresh(false); });");
