...
export function WorldbuildingWorkspace({ repository, snapshot, tabs, activeTabKey, onOpenTab, onActivateTab, onCloseTab, onCreate, onRefresh, onRegisterNoteFlush, onFlushActiveNote, onReturnToManuscript, onError }: { repository: ProjectRepository; snapshot: ProjectSnapshot; tabs: WorldbuildingTab[]; activeTabKey?: string; onOpenTab: (tab: WorldbuildingTab, options?: { noteFlushed?: boolean }) => void; onActivateTab: (key: string) => void; onCloseTab: (key: string) => void; onCreate: () => void; onRefresh: () => Promise<void>; onRegisterNoteFlush: (noteId: string, flush?: () => Promise<boolean>) => void; onFlushActiveNote: (noteId: string) => Promise<boolean>; onReturnToManuscript: () => void; onError: (message: string) => void }) {
  ...
  return <main id="worldbuilding-workspace" role="tabpanel" aria-labelledby="worldbuilding-workspace-tab" className="worldbuilding-workspace"><section className="worldbuilding-main">{tabs.length > 0 && <TabStrip tabs={tabs} activeKey={active ? worldbuildingTabKey(active) : undefined} snapshot={snapshot} onActivate={onActivateTab} onClose={onCloseTab} />}{note ? <NoteEditor repository={repository} note={note} snapshot={snapshot} onRefresh={onRefresh} onOpen={(id) => { void openNote(id); }} onRegisterNoteFlush={onRegisterNoteFlush} onError={onError} /> : canvas ? engine === 'excalidraw' ? <ExcalidrawCanvas repository={repository} canvas={canvas} onRefresh={onRefresh} onError={onError} /> : <NoteCanvas repository={repository} canvas={canvas} snapshot={snapshot} onRefresh={onRefresh} onError={onError} /> : <section className="worldbuilding-empty" aria-label="Worldbuilding empty state"><p className="eyebrow">LOCAL WORLDBUILDING</p><p>Create a Markdown page or choose a canvas engine.</p><button type="button" onClick={onCreate}>Create in Worldbuilding</button><button type="button" onClick={onReturnToManuscript}>Close</button></section>}</section></main>;
}
