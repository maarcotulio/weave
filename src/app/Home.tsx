...
export function HomePage({ stats, activity, onOpenManuscript }: { stats: WritingStats; activity: WritingActivity[]; onOpenManuscript: () => void }) {
  ...
  return <main id="home-workspace" className="home-page" role="tabpanel" aria-label="Home">
    ...
    <section className="home-stat-grid" aria-label="Writing overview"><article><span>Today</span><strong>{stats.dailyWords.toLocaleString()} <small>/ {stats.dailyTarget.toLocaleString()} words</small></strong><div className="home-progress"><span style={{ width: `${progress}%` }} /></div></article><article><span>Project total</span><strong>{stats.projectWords.toLocaleString()} <small>words</small></strong></article><article><span>Daily target</span><strong>{stats.dailyTarget.toLocaleString()} <small>words</small></strong></article></section>
