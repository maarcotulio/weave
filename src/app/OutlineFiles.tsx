...
export function OutlineFiles({ files, busy, onCreate, onSave, onDelete }: { files: OutlineFile[]; busy: boolean; onCreate: (title: string) => void; onSave: (file: OutlineFile, markdown: string) => void; onDelete: (file: OutlineFile) => void }) {
  const [activeId, setActiveId] = useState<string>();
  const active = files.find((file) => file.id === activeId) ?? files[0];
  const [markdown, setMarkdown] = useState(active?.markdown ?? '');
  useEffect(() => { setMarkdown(active?.markdown ?? ''); }, [active?.id, active?.markdown]);
  ...
  return <section className="outline-files" aria-label="Outline files">
    ...
    <div className="outline-files-grid"><nav aria-label="Outline files">{files.map((file) => <button type="button" key={file.id} className={active?.id === file.id ? 'selected' : ''} onClick={() => setActiveId(file.id)}>{file.title}</button>)}{!files.length && <p>No outline files yet.</p>}</nav>
      {active && <div className="outline-file-editor"><label>Markdown<textarea aria-label={`${active.title} Markdown`} value={markdown} onChange={(event) => setMarkdown(event.target.value)} /></label><div><button type="button" className="text-button" disabled={busy} onClick={() => onDelete(active)}>Delete</button><button type="button" className="primary-button" disabled={busy || markdown === active.markdown} onClick={() => onSave(active, markdown)}>Save outline</button></div></div>}
    </div>
  </section>;
}
