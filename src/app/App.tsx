...
export default function App() {
  ...
  const createWorldbuildingNote = () => void run(async () => { const note = await repository.createMarkdownNote('Untitled note'); await refresh(); openWorldbuildingTab({ kind: 'note', id: note.id }); });
  const createWorldbuildingCanvas = () => { if (!selectedStoryId) { setLocalError('Choose a manuscript story before creating a local note canvas.'); return; } const title = window.prompt('Canvas title', 'Untitled canvas'); if (!title?.trim()) return; void run(async () => { const canvas = await repository.createCanvas(selectedStoryId, title.trim()); await refresh(); openWorldbuildingTab({ kind: 'canvas', id: canvas.id }); }); };
