...
function worldbuildingTabKeysDeletedByFolder(snapshot: ProjectSnapshot, folderId: string): Set<string> {
  const folderIds = new Set([folderId]);
  let changed = true;
  while (changed) {
    changed = false;
    for (const folder of snapshot.worldbuildingFolders ?? []) {
  ...
  const noteIds = new Set(snapshot.markdownNotes.filter((note) => note.folderId && folderIds.has(note.folderId)).map((note) => note.id));
  const canvasIds = new Set(snapshot.canvases.filter((canvas) => canvas.folderId && folderIds.has(canvas.folderId)).map((canvas) => canvas.id));
  return new Set([
    ...[...noteIds].map((id) => worldbuildingTabKey({ kind: 'note', id })),
    ...[...canvasIds].map((id) => worldbuildingTabKey({ kind: 'canvas', id }))
  ]);
}
...
import { OutlineFiles } from './OutlineFiles';
import { ProjectFolderImportDialog } from './ProjectFolderImportDialog';
import { ImportChoiceDialog } from './ImportChoiceDialog';
import { MarkdownImportAction } from './MarkdownImportAction';
...
type FolderActionState =
  | { kind: 'rename'; folderId: string; title: string; trigger?: FocusTrigger }
  | { kind: 'delete'; folderId: string; title: string; trigger?: FocusTrigger; error?: string };
...
function WorldbuildingTree({ snapshot, activeKey, error, onOpen, onCreate, onRenameFolder, onDeleteFolder, onRenameNote, onDeleteNote, onRenameCanvas, onDeleteCanvas, onMove }: { snapshot: ProjectSnapshot; activeKey?: string; error?: string; onOpen: (entry: WorldbuildingEntry) => void; onCreate: () => void; onRenameFolder: (folder: WorldbuildingFolder) => void; onDeleteFolder: (folder: WorldbuildingFolder) => void; onRenameNote: (note: MarkdownNote) => void; onDeleteNote: (note: MarkdownNote) => void; onRenameCanvas: (canvas: StoryCanvas) => void; onDeleteCanvas: (canvas: StoryCanvas) => void; onMove: (entry: WorldbuildingEntry, parentId?: string) => void }) {
  const folders = snapshot.worldbuildingFolders ?? [];
  const [expanded, setExpanded] = useState<Set<string>>(() => new Set(folders.map((folder) => folder.id)));
  const [dragging, setDragging] = useState<WorldbuildingEntry>();
  const [moving, setMoving] = useState<WorldbuildingEntry>();
  const [moveDestination, setMoveDestination] = useState('');
  const entries = (parentId?: string): WorldbuildingEntry[] => [...folders.filter((folder) => folder.parentId === parentId).map((folder) => ({ kind: 'folder' as const, id: folder.id, title: folder.title, parentId: folder.parentId, position: folder.position })), ...snapshot.markdownNotes.filter((note) => note.folderId === parentId).map((note) => ({ kind: 'note' as const, id: note.id, title: note.title, parentId: note.folderId, position: note.position ?? 0 })), ...snapshot.canvases.filter((canvas) => canvas.folderId === parentId).map((canvas) => ({ kind: 'canvas' as const, id: canvas.id, title: canvas.title, parentId: canvas.folderId, position: canvas.position ?? 0 }))].sort((left, right) => left.position - right.position || left.kind.localeCompare(right.kind) || left.id.localeCompare(right.id));
  const invalidFolderDestination = (sourceId: string, destinationId: string) => { let current: string | undefined = destinationId; while (current) { if (current === sourceId) return true; current = folders.find((folder) => folder.id === current)?.parentId; } return false; };
  const beginMove = (entry: WorldbuildingEntry) => { setMoving(entry); setMoveDestination(entry.parentId ?? ''); };
  const submitMove = () => { if (!moving) return; onMove(moving, moveDestination || undefined); setMoving(undefined); };
  const beginDrag = (event: React.DragEvent, entry: WorldbuildingEntry) => { event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('application/x-weave-worldbuilding', JSON.stringify({ kind: entry.kind, id: entry.id })); setDragging(entry); };
  const dropOn = (event: React.DragEvent, parentId?: string) => { event.preventDefault(); event.stopPropagation(); const raw = event.dataTransfer.getData('application/x-weave-worldbuilding'); if (!raw) return; try { const value = JSON.parse(raw) as { kind: WorldbuildingEntry['kind']; id: string }; onMove({ kind: value.kind, id: value.id, title: '', parentId, position: 0 }, parentId); } catch { /* ignore malformed browser drag payload */ } setDragging(undefined); };
  const row = (entry: WorldbuildingEntry, depth = 0): React.ReactNode => {
    const selected = activeKey === `${entry.kind}:${entry.id}`;
    const folder = entry.kind === 'folder' ? folders.find((candidate) => candidate.id === entry.id) : undefined;
    const childEntries = entry.kind === 'folder' ? entries(entry.id) : [];
    return <React.Fragment key={`${entry.kind}:${entry.id}`}><div className={`worldbuilding-file-row ${selected ? 'selected' : ''} ${dragging?.id === entry.id ? 'dragging' : ''}`} style={{ '--tree-depth': depth } as React.CSSProperties} draggable onDragStart={(event) => beginDrag(event, entry)} onDragEnd={() => setDragging(undefined)} onDragOver={(event) => { if (entry.kind === 'folder') event.preventDefault(); }} onDrop={(event) => { if (entry.kind === 'folder') dropOn(event, entry.id); }}>
      {entry.kind === 'folder' ? <button type="button" className="worldbuilding-file-toggle" aria-label={`${expanded.has(entry.id) ? 'Collapse' : 'Expand'} ${entry.title}`} onClick={() => setExpanded((current) => { const next = new Set(current); if (next.has(entry.id)) next.delete(entry.id); else next.add(entry.id); return next; })}>{expanded.has(entry.id) ? '▾' : '▸'}</button> : <span className="worldbuilding-file-spacer" />}
      <button type="button" className="worldbuilding-file-open" aria-current={selected ? 'page' : undefined} onClick={() => entry.kind !== 'folder' && onOpen(entry)}>{entry.title}</button>{entry.kind === 'canvas' && <span className="worldbuilding-canvas-flag">(canvas)</span>}
      <div className="worldbuilding-file-actions"><button type="button" aria-label={`Move ${entry.title}`} title={`Move ${entry.title}`} onClick={() => beginMove(entry)}>↗</button>{entry.kind === 'folder' ? <><button type="button" aria-label={`Rename ${entry.title}`} title={`Rename ${entry.title}`} onClick={() => folder && onRenameFolder(folder)}><Pencil size={13} aria-hidden="true" /></button><button type="button" aria-label={`Delete ${entry.title}`} title={`Delete ${entry.title}`} onClick={() => folder && onDeleteFolder(folder)}><Trash2 size={13} aria-hidden="true" /></button></> : entry.kind === 'note' ? <><button type="button" aria-label={`Rename ${entry.title}`} title={`Rename ${entry.title}`} onClick={() => onRenameNote(snapshot.markdownNotes.find((note) => note.id === entry.id)!)}><Pencil size={13} aria-hidden="true" /></button><button type="button" aria-label={`Delete ${entry.title}`} title={`Delete ${entry.title}`} onClick={() => onDeleteNote(snapshot.markdownNotes.find((note) => note.id === entry.id)!)}><Trash2 size={13} aria-hidden="true" /></button></> : <><button type="button" aria-label={`Rename ${entry.title}`} title={`Rename ${entry.title}`} onClick={() => onRenameCanvas(snapshot.canvases.find((canvas) => canvas.id === entry.id)!)}><Pencil size={13} aria-hidden="true" /></button><button type="button" aria-label={`Delete ${entry.title}`} title={`Delete ${entry.title}`} onClick={() => onDeleteCanvas(snapshot.canvases.find((canvas) => canvas.id === entry.id)!)}><Trash2 size={13} aria-hidden="true" /></button></>}{moving?.kind === entry.kind && moving.id === entry.id && <span className="worldbuilding-file-move"><label>To <select aria-label={`Move ${entry.title} to folder`} value={moveDestination} onChange={(event) => setMoveDestination(event.target.value)}><option value="">Root</option>{folders.filter((candidate) => candidate.id !== entry.id && (entry.kind !== 'folder' || !invalidFolderDestination(entry.id, candidate.id))).map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.title}</option>)}</select></label><button type="button" aria-label={`Confirm move of ${entry.title}`} onClick={submitMove}>Move</button><button type="button" aria-label={`Cancel move of ${entry.title}`} onClick={() => setMoving(undefined)}>Cancel</button></span>}</div>
    </div>{entry.kind === 'folder' && expanded.has(entry.id) && childEntries.map((child) => row(child, depth + 1))}</React.Fragment>;
  };
  return <section className="worldbuilding-file-tree" aria-label="Worldbuilding files" onDragOver={(event) => event.preventDefault()} onDrop={(event) => dropOn(event)}>{error && <div className="inline-error" role="alert">{error}</div>}<div className="worldbuilding-file-tree-head"><h2>Files</h2><div className="worldbuilding-file-tree-actions"><button type="button" className="worldbuilding-create-button" onClick={onCreate}><Plus size={14} aria-hidden="true" /> Create</button></div></div><div className="worldbuilding-file-root" aria-label="Worldbuilding root drop target" onDragOver={(event) => event.preventDefault()} onDrop={(event) => dropOn(event)}><span>Root</span></div>{entries().map((entry) => row(entry))}{entries().length === 0 && <p className="worldbuilding-file-empty">No notes, canvases, or folders yet.</p>}</section>;
}

function WorldbuildingCreateDialog({ busy, onCancel, onChoice, trigger }: { busy: boolean; onCancel: () => void; onChoice: (choice: 'note' | 'canvas' | 'folder') => void; trigger?: FocusTrigger }) {
  return <Modal eyebrow="WORLDBUILDING" title="Create in Worldbuilding" onClose={onCancel} closeDisabled={busy} busy={busy} trigger={trigger} descriptionId="worldbuilding-create-description"><p id="worldbuilding-create-description">Choose one local file type to create. Notes and canvases appear together in the Worldbuilding file tree.</p><div className="choice-grid"><button type="button" disabled={busy} onClick={() => onChoice('note')}><strong>Markdown note</strong><span>Write canonical Markdown with existing autosave and wiki links.</span></button><button type="button" disabled={busy} onClick={() => onChoice('canvas')}><strong>Canvas</strong><span>Choose the existing React Flow or Excalidraw engine.</span></button><button type="button" disabled={busy} onClick={() => onChoice('folder')}><strong>Folder</strong><span>Organize notes, canvases, and nested folders.</span></button></div><div className="modal-actions"><button type="button" className="text-button" disabled={busy} onClick={onCancel}>Cancel</button></div></Modal>;
}

function FolderCreateDialog({ busy, onCancel, onSubmit, trigger }: { busy: boolean; onCancel: () => void; onSubmit: (title: string) => void; trigger?: FocusTrigger }) {
  const [value, setValue] = useState('');
  return <Modal eyebrow="WORLDBUILDING FOLDER" title="Create folder" onClose={onCancel} closeDisabled={busy} busy={busy} trigger={trigger}><form onSubmit={(event) => { event.preventDefault(); if (value.trim()) onSubmit(value.trim()); }}><label className="modal-field">Folder name<input autoFocus required value={value} onChange={(event) => setValue(event.target.value)} /></label><div className="modal-actions"><button type="button" className="text-button" disabled={busy} onClick={onCancel}>Cancel</button><button type="submit" className="primary-button" disabled={busy || !value.trim()}>Create folder</button></div></form></Modal>;
}

function FolderRenameDialog({ initialTitle, busy, onCancel, onSubmit, trigger }: { initialTitle: string; busy: boolean; onCancel: () => void; onSubmit: (title: string) => void; trigger?: FocusTrigger }) {
  const [value, setValue] = useState(initialTitle);
  useEffect(() => setValue(initialTitle), [initialTitle]);
  return <Modal eyebrow="WORLDBUILDING FOLDER" title="Rename folder" onClose={onCancel} closeDisabled={busy} busy={busy} trigger={trigger}><form onSubmit={(event) => { event.preventDefault(); if (value.trim()) onSubmit(value.trim()); }}><label className="modal-field">Folder name<input autoFocus required value={value} onChange={(event) => setValue(event.target.value)} /></label><div className="modal-actions"><button type="button" className="text-button" disabled={busy} onClick={onCancel}>Cancel</button><button type="submit" className="primary-button" disabled={busy || !value.trim()}>Rename folder</button></div></form></Modal>;
}

function FolderDeleteDialog({ folder, busy, onCancel, onConfirm, trigger }: { folder: FolderActionState & { kind: 'delete' }; busy: boolean; onCancel: () => void; onConfirm: () => void; trigger?: FocusTrigger }) {
  return <Modal eyebrow="DESTRUCTIVE WORLDBUILDING ACTION" title={`Delete “${folder.title}”?`} onClose={onCancel} closeDisabled={busy} busy={busy} trigger={trigger} descriptionId="delete-worldbuilding-folder-description"><p id="delete-worldbuilding-folder-description">This permanently deletes the folder, every nested folder, all contained notes and canvases, their canvas placements, and derived edges. Existing incoming Markdown links remain unresolved and repairable.</p>{folder.error && <p className="error-message" role="alert">{folder.error}</p>}<div className="modal-actions"><button type="button" className="text-button" autoFocus disabled={busy} onClick={onCancel}>Cancel</button><button type="button" className="danger-button" disabled={busy} onClick={onConfirm}>Delete folder and contents</button></div></Modal>;
}
...
export default function App() {
  ...
  const [worldbuildingCreateDialog, setWorldbuildingCreateDialog] = useState<{ trigger?: FocusTrigger }>();
  const [folderCreateDialog, setFolderCreateDialog] = useState<{ trigger?: FocusTrigger }>();
  const [folderAction, setFolderAction] = useState<FolderActionState>();
  ...
  const chooseProjectFolderImport = () => { if (!isDesktop) return; const trigger = importChoiceDialog?.trigger; setImportChoiceDialog(undefined); setProjectFolderImportDialog({ trigger }); };
  const chooseMarkdownImport = () => { const trigger = importChoiceDialog?.trigger; setImportChoiceDialog(undefined); setMarkdownImportDialog({ defaultTarget: selectedChapterId ? 'scene' : 'chapter', trigger }); };
  const chooseProjectFolder = () => void run(async () => {
    const directory = await openDirectory({ directory: true, multiple: false, title: 'Choose project folder to import' });
    if (!directory || Array.isArray(directory)) return;
    const project = await repository.importProjectDirectory(directory);
    rememberProject(project); setProjectFolderImportDialog(undefined); await refresh(false); applyRoute('home');
  });
  ...
  const importMarkdownFiles = (files: ImportedMarkdownFile[], target: MarkdownImportTarget) => void run(async () => {
    ...
    if (target === 'scene') {
      ...
      setMode('scene');
      await refresh(true);
      setSelectedSceneId(importedScenes.at(-1)?.id);
      setToast({ message: `Imported ${importedScenes.length} Markdown scene${importedScenes.length === 1 ? '' : 's'}.` });
      return;
    }
  ...
  const openWorldbuildingCreate = () => {
    const trigger = captureFocusTrigger();
    return void run(async () => { await autosave.flush(); setWorldbuildingCreateDialog({ trigger }); });
  };
  const createWorldbuildingNote = (trigger = captureFocusTrigger()) => void run(async () => { await autosave.flush(); setWorldbuildingCreateDialog(undefined); setNoteCreateDialog({ trigger }); });
  const createWorldbuildingFolder = (trigger = captureFocusTrigger()) => void run(async () => { await autosave.flush(); setWorldbuildingCreateDialog(undefined); setFolderCreateDialog({ trigger }); });
  ...
  const requestRenameFolder = (folder: WorldbuildingFolder) => setFolderAction({ kind: 'rename', folderId: folder.id, title: folder.title, trigger: captureFocusTrigger() });
  const requestDeleteFolder = (folder: WorldbuildingFolder) => setFolderAction({ kind: 'delete', folderId: folder.id, title: folder.title, trigger: captureFocusTrigger() });
  const submitFolderRename = (title: string) => void run(async () => { const action = folderAction; if (!action || action.kind !== 'rename') return; await repository.renameWorldbuildingFolder(action.folderId, title); setFolderAction(undefined); await refresh(); });
  const confirmFolderDelete = () => void run(async () => {
    const action = folderAction;
    if (!action || action.kind !== 'delete') return;
    if (!await flushActiveMarkdownNote()) return;
    if (!snapshot) return;
    const deletedTabKeys = worldbuildingTabKeysDeletedByFolder(snapshot, action.folderId);
    try {
      await repository.deleteWorldbuildingFolder(action.folderId);
      setFolderAction(undefined);
      setWorldbuildingTabs((current) => {
        const activeIndex = current.findIndex((tab) => worldbuildingTabKey(tab) === activeWorldbuildingTabKey);
        const next = current.filter((tab) => !deletedTabKeys.has(worldbuildingTabKey(tab)));
        if (activeIndex >= 0 && activeWorldbuildingTabKey && deletedTabKeys.has(activeWorldbuildingTabKey)) {
          const fallback = next[activeIndex] ?? next[activeIndex - 1];
          setActiveWorldbuildingTabKey(fallback ? worldbuildingTabKey(fallback) : undefined);
        }
        return next;
      });
      await refresh();
    } catch (error) {
      setFolderAction({ ...action, error: error instanceof Error ? error.message : String(error) });
    }
  });
  const moveWorldbuildingEntry = (entry: WorldbuildingEntry, parentId?: string) => void run(async () => { try { await repository.moveWorldbuildingEntry(entry.kind, entry.id, parentId); await refresh(); } catch (error) { setLocalError(error instanceof Error ? error.message : String(error)); } });
  ...
  if (!snapshot) return <><main className="welcome"><div className="welcome-card"><div className="welcome-card-top"><div className="mark">W</div><ThemeControl theme={theme} onToggle={() => setTheme(toggleTheme)} /></div><p className="eyebrow">OFFLINE DESKTOP WRITING</p><h1>Make room for the story.</h1>{isDesktop ? <p className="welcome-copy">Weave keeps your manuscript, revisions, SQLite database, and recovery files in a visible <code>.weave</code> project directory. No server. No network. No guesswork.</p> : <p className="modal-warning" role="alert">Browser preview — nothing is saved. A project made here lives only in this tab's memory: no <code>.weave</code> folder, no database, nothing on disk, and it's gone on refresh. Run <code>npm run desktop:dev</code> for the real, persistent desktop app.</p>}<div className="welcome-actions"><button type="button" className="primary-button" onClick={createProject} disabled={busy}>Create project</button><button type="button" className="secondary-button" onClick={openProject} disabled={busy}>Open project</button></div>{validRecentProjects.length > 0 && <section className="recent-projects" aria-labelledby="recent-projects-title"><div className="recent-projects-heading"><h2 id="recent-projects-title">Recent projects</h2><span>local only</span></div><ul>{validRecentProjects.map((project) => <li key={project.directory}><button type="button" className="recent-project-open" onClick={() => openProjectAt(project.directory, project.name)} disabled={busy}><strong>{project.name}</strong><small>{project.directory}</small></button><button type="button" className="recent-project-remove" onClick={() => forgetRecentProject(project.directory)} disabled={busy} aria-label={`Remove ${project.name} from recent projects`}>Remove</button></li>)}</ul></section>}{localError && <div className="error-message" role="alert"><p>{localError}</p>{projectPickerRetry && <button type="button" className="retry-button" onClick={() => projectPickerRetry === 'create' ? createProject() : openProject()}>Retry</button>}</div>}<p className="offline-note"><span className="status-dot" /> {isDesktop ? 'local-only · SQLite · revisioned' : 'browser preview · nothing persisted'}</p></div></main>{formDialog && <FormDialog config={formDialog} busy={busy} error={localError} onCancel={() => { setFormDialog(undefined); setLocalError(''); }} onSubmit={submitFormDialog} />}</>;
  ...
  return <div className={`app-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''} ${focusMode ? 'focus-mode' : ''}`}>
    ...
    {route === 'home' ? <HomePage stats={snapshot.writingStats} activity={snapshot.writingActivity} onOpenManuscript={() => navigate('manuscript')} onImportProject={openImportChoices} importAvailable={true} error={localError} /> : route === 'settings' ? <SettingsPage styleProfile={styleProfile} stats={snapshot.writingStats} backups={snapshot.backups} integrityReport={integrityReport} lastBackup={lastBackup} busy={busy} onSaveGoal={updateGoal} onSaveStyle={updateStyle} onCheckIntegrity={checkIntegrity} onCreateBackup={backup} onRecoverBackup={recoverBackup} /> : route === 'outline' ? <>{localError && <div className="inline-error" role="alert">{localError}<button type="button" className="retry-button" onClick={openImportChoices}>Retry import</button></div>}<ManuscriptOutline snapshot={snapshot} selectedChapterId={selectedChapterId} selectedSceneId={selectedSceneId} outlineFiles={<OutlineFiles files={snapshot.outlineFiles ?? []} busy={busy} onCreate={createOutlineFile} onSave={saveOutlineFile} onDelete={deleteOutlineFile} />} onSelectChapter={selectOutlineChapter} onSelectScene={selectOutlineScene} onMoveChapter={moveChapter} onMoveScene={(scene, position) => moveScene(scene, position - scene.position)} /></> : workspaceMode === 'worldbuilding' ? <WorldbuildingWorkspace repository={repository} snapshot={snapshot} tabs={worldbuildingTabs} activeTabKey={activeWorldbuildingTabKey} onOpenTab={openWorldbuildingTab} onActivateTab={activateWorldbuildingTab} onCloseTab={closeWorldbuildingTab} onCreate={openWorldbuildingCreate} onRefresh={async () => { await refresh(); }} onRegisterNoteFlush={registerNoteFlush} onFlushActiveNote={flushNoteBeforeAction} onReturnToManuscript={() => navigate('manuscript')} onError={setLocalError} /> : <main id="manuscript-workspace" role="tabpanel" aria-labelledby="manuscript-workspace-tab" className="workspace"><div className="workspace-head"><div><p className="eyebrow">{activeChapter?.title ?? 'Chapter'}</p><h1>{mode === 'continuous' ? 'Continuous draft' : mode === 'compose' ? 'Composed chapter' : activeScene?.title ?? 'Choose a scene'}</h1></div><div className="workspace-manuscript-actions"><div className="manuscript-version-actions"><button type="button" className="secondary-button" disabled={busy} onClick={() => openManuscriptVersionDialog('save')}>Save version</button><button type="button" className="icon-button version-help-button" disabled={busy} aria-label="Explain manuscript versions" title="About manuscript versions" onClick={() => openManuscriptVersionDialog('help')}><CircleHelp size={16} aria-hidden="true" /></button>{snapshot.manuscriptVersions.length > 0 && <button type="button" className="text-button version-history-button" disabled={busy} onClick={() => openManuscriptVersionDialog('history')}>Versions ({snapshot.manuscriptVersions.length})</button>}</div><div className="mode-switch"><button type="button" className={mode === 'scene' ? 'active' : ''} onClick={() => activeScene && selectScene(activeScene)}>Scenes</button><button type="button" className={mode === 'compose' ? 'active' : ''} onClick={compose}>Chapter view</button><button type="button" className={mode === 'continuous' ? 'active' : ''} onClick={openContinuous}>Continuous draft</button></div></div></div>
      ...
      <div className="page-stack" ref={pageStackRef} aria-label="Manuscript pages">{mode === 'scene' && !activeScene ? <section className="manuscript-scene-empty" aria-label="No scene selected"><p className="eyebrow">MANUSCRIPT</p><p>{selectedChapterId ? 'This chapter has no scenes yet. Create one to start writing.' : 'Choose or create a chapter, then add a scene to start writing.'}</p>{selectedChapterId && <button type="button" onClick={addScene}>+ New scene</button>}</section> : <><StyleControls profile={styleProfile} onChange={updateStyle} disabled={mode === 'compose'} focusMode={focusMode} onToggleFocus={toggleFocusMode} />{pages.map((page, pageIndex) => <section className="paper-page" style={pageStyle} key={`${pageIndex}-${page.document.blocks[0]?.id ?? 'empty'}`}><div className="paper-meta"><span>{mode === 'compose' ? 'NON-DESTRUCTIVE COMPOSITION' : mode === 'continuous' ? 'SEPARATE REVISION · SOURCE SNAPSHOT PRESERVED' : 'SCENE DOCUMENT'}</span><span>{pageDimensions(styleProfile.pageSize).label} · Page {pageIndex + 1} of {pages.length}</span></div><Editor document={page.document} styleProfile={styleProfile} onChange={(value) => updatePage(pageIndex, value)} onSelectionChange={rememberSelection} readOnly={mode === 'compose'} /><div className="paper-footer"><span>{mode === 'compose' ? 'Scene documents remain the source.' : 'Structured document · changes save automatically'}</span>{documentHead && pageIndex === pages.length - 1 && <span>revision {documentHead.revision}</span>}</div></section>)}</>}</div>
      {(mode !== 'scene' || activeScene) && <footer className="editor-footer"><div>{mode === 'continuous' && <button type="button" className="secondary-button" onClick={returnToScenes}>Return to scenes</button>}{mode === 'scene' && activeScene && <><button type="button" className="secondary-button" onClick={() => renameScene(activeScene)}>Rename</button><button type="button" className="icon-button" onClick={() => moveScene(activeScene, -1)} aria-label="Move scene up">↑</button><button type="button" className="icon-button" onClick={() => moveScene(activeScene, 1)} aria-label="Move scene down">↓</button></>}{mode === 'compose' && <span className="guard-note">Composition is a view, never a second source.</span>}</div><div className="save-actions">{mode !== 'compose' && <button type="button" className="secondary-button" onClick={save} disabled={busy || !documentHead}>Save now</button>}</div></footer>}
    </main>}
    ...
    {folderAction?.kind === 'rename' && <FolderRenameDialog initialTitle={folderAction.title} busy={busy} trigger={folderAction.trigger} onCancel={() => setFolderAction(undefined)} onSubmit={submitFolderRename} />}
    {folderAction?.kind === 'delete' && <FolderDeleteDialog folder={folderAction} busy={busy} trigger={folderAction.trigger} onCancel={() => setFolderAction(undefined)} onConfirm={confirmFolderDelete} />}
