...
function WorldbuildingTree({ snapshot, activeKey, error, onOpen, onCreate, onRenameFolder, onDeleteFolder, onRenameNote, onDeleteNote, onRenameCanvas, onDeleteCanvas, onMove }: { snapshot: ProjectSnapshot; activeKey?: string; error?: string; onOpen: (entry: WorldbuildingEntry) => void; onCreate: () => void; onRenameFolder: (folder: WorldbuildingFolder) => void; onDeleteFolder: (folder: WorldbuildingFolder) => void; onRenameNote: (note: MarkdownNote) => void; onDeleteNote: (note: MarkdownNote) => void; onRenameCanvas: (canvas: StoryCanvas) => void; onDeleteCanvas: (canvas: StoryCanvas) => void; onMove: (entry: WorldbuildingEntry, parentId?: string) => void }) {
  ...
  return <section className="worldbuilding-file-tree" aria-label="Worldbuilding files" onDragOver={(event) => event.preventDefault()} onDrop={(event) => dropOn(event)}>{error && <div className="inline-error" role="alert">{error}</div>}<div className="worldbuilding-file-tree-head"><h2>Worldbuilding files</h2><button type="button" className="worldbuilding-create-button" onClick={onCreate}><Plus size={14} aria-hidden="true" /> Create</button></div><div className="worldbuilding-file-root" aria-label="Worldbuilding root drop target" onDragOver={(event) => event.preventDefault()} onDrop={(event) => dropOn(event)}><span>Root</span></div>{entries().map((entry) => row(entry))}{entries().length === 0 && <p className="worldbuilding-file-empty">No notes, canvases, or folders yet.</p>}</section>;
}
