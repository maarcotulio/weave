...
function MarkdownImportDialog({ defaultTarget, canImportScene, canImportChapter, busy, onCancel, onSubmit, trigger }: { defaultTarget: MarkdownImportTarget; canImportScene: boolean; canImportChapter: boolean; busy: boolean; onCancel: () => void; onSubmit: (files: ImportedMarkdownFile[], target: MarkdownImportTarget) => void; trigger?: FocusTrigger }) {
  ...
  return <Modal eyebrow="MARKDOWN IMPORT" title={title} onClose={onCancel} closeDisabled={busy} busy={busy} trigger={trigger} descriptionId={descriptionId}>
    <p id={descriptionId}>Choose one or more .md files. Each file becomes a new scene, or a new chapter with one scene containing the file content.</p>
    <label className="modal-field markdown-file-picker"><span>Markdown files</span><span className="markdown-file-control"><span className="markdown-file-button">Choose .md files</span><span className="markdown-file-hint">{files.length ? `${files.length} file${files.length === 1 ? '' : 's'} selected` : 'No files selected'}</span></span><input type="file" accept=".md,text/markdown" multiple aria-label="Choose Markdown files" onChange={(event) => void chooseFiles(event)} /></label>
    {error && <p className="error-message" role="alert">{error}</p>}
    {files.length > 0 && <ul className="markdown-import-files" aria-label="Selected Markdown files">{files.map((file, index) => <li key={`${file.name}-${index}`}>{file.name}</li>)}</ul>}
    ...
    <div className="modal-actions"><button type="button" className="text-button" disabled={busy} onClick={onCancel}>Cancel</button><button type="button" className="primary-button" disabled={busy || !ready} onClick={() => onSubmit(files, target)}>Import {files.length || ''} file{files.length === 1 ? '' : 's'}</button></div>
  </Modal>;
}
...
export default function App() {
  ...
  const importProjectFolder = () => { if (!isDesktop) return; setProjectFolderImportDialog({ trigger: captureFocusTrigger() }); };
