...
export default function App() {
  ...
  const saveOutlineFile = (file: import('../domain/types').OutlineFile, markdown: string) => void run(async () => { await repository.updateOutlineFile(file.id, { title: file.title, markdown }, file.revision); await refresh(); });
  ...
  return <div className={`app-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''} ${focusMode ? 'focus-mode' : ''}`}>
    ...
    {projectFolderImportDialog && <ProjectFolderImportDialog busy={busy} trigger={projectFolderImportDialog.trigger} onCancel={() => setProjectFolderImportDialog(undefined)} onChoose={chooseProjectFolder} />}
