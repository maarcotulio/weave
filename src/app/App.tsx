...
import { DEFAULT_EDITOR_STYLE, FONT_FAMILY_OPTIONS, FONT_SIZE_OPTIONS, LINE_SPACING_OPTIONS, PAGE_SIZE_OPTIONS, type Chapter, type ContinuousDraft, type DocumentAnchor, type EditorStyleProfile, type ExportFormat, type ProjectSnapshot, type Scene, type SemanticMark, type StructuredDocument, type WritingStats } from '../domain/types';
...
export default function App() {
  ...
  const [sourceContextAnchor, setSourceContextAnchor] = useState<DocumentAnchor>();
  ...
  useEffect(() => {
    if (!sourceContextAnchor || mode !== 'scene' || documentHead?.documentId !== sourceContextAnchor.documentId) return;
    const field = Array.from(document.querySelectorAll<HTMLTextAreaElement>('textarea[data-source-block-id]')).find((element) => element.dataset.sourceBlockId === sourceContextAnchor.blockId);
    if (!field) return;
    field.focus();
    field.setSelectionRange(Math.min(sourceContextAnchor.start, field.value.length), Math.min(sourceContextAnchor.end, field.value.length));
    setSourceContextAnchor(undefined);
  }, [documentHead?.documentId, mode, pages, sourceContextAnchor]);
  ...
  return <div className={`app-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
    ...
    <aside className="sidebar"><nav className="workspace-sections" aria-label="Primary workspaces"><button type="button" className={workspaceMode === 'writing' ? 'selected' : ''} aria-current={workspaceMode === 'writing' ? 'page' : undefined} onClick={() => void run(async () => { await autosave.flush(); setWorkspaceMode('writing'); })}>Manuscript</button><button type="button" className={workspaceMode === 'worldbuilding' ? 'selected' : ''} aria-current={workspaceMode === 'worldbuilding' ? 'page' : undefined} onClick={() => void run(async () => { await autosave.flush(); setWorkspaceMode('worldbuilding'); })}>Worldbuilding</button></nav>{workspaceMode === 'writing' ? <><div className="sidebar-heading"><span>MANUSCRIPT</span><div className="sidebar-heading-actions"><button type="button" onClick={createProject} aria-label="New project">+</button><button type="button" onClick={() => setSidebarCollapsed(true)} aria-label="Collapse sidebar">‹</button></div></div>{snapshot.stories.map((story) => <div key={story.id} className="tree-group"><button type="button" className={`tree-item story ${selectedStoryId === story.id ? 'selected' : ''}`} onClick={() => void run(async () => { await autosave.flush(); setSelectedStoryId(story.id); })}>▾ <span>{story.title}</span></button>{snapshot.chapters.filter((chapter) => chapter.storyId === story.id).map((chapter) => <div key={chapter.id} className="chapter-group"><button type="button" className={`tree-item chapter ${selectedChapterId === chapter.id ? 'selected' : ''}`} onClick={() => void run(async () => { await autosave.flush(); setSelectedChapterId(chapter.id); setSelectedSceneId(undefined); })}>▾ <span>{chapter.title}</span></button>{selectedChapterId === chapter.id && scenes.map((scene) => <button type="button" key={scene.id} className={`tree-item scene ${selectedSceneId === scene.id && mode === 'scene' ? 'selected' : ''}`} onClick={() => selectScene(scene)}><span className="scene-index">{String(scene.position + 1).padStart(2, '0')}</span>{scene.title}</button>)}{selectedChapterId === chapter.id && <button type="button" className="add-scene" onClick={addScene}>+ New scene</button>}</div>)}</div>)}<button type="button" className="add-story" onClick={newStory}>+ New story</button><div className="sidebar-bottom"><button type="button" onClick={addChapter} disabled={!selectedStoryId}>+ Chapter</button><button type="button" onClick={openProject}>Open</button></div></> : <section className="worldbuilding-sidebar" aria-label="Worldbuilding section"><p className="eyebrow">WORLDBUILDING</p><strong>Linked local notes</strong><span>{snapshot.markdownNotes.length} Markdown note{snapshot.markdownNotes.length === 1 ? '' : 's'}</span><strong>Structured items</strong><span>{snapshot.worldbuildingItems.length} worldbuilding item{snapshot.worldbuildingItems.length === 1 ? '' : 's'}</span><strong>User-created canvases</strong><span>{snapshot.canvases.length} saved canvas{snapshot.canvases.length === 1 ? '' : 'es'}</span><p>Notes, links, and canvases remain local to this project.</p></section>}</aside>
    {workspaceMode === 'worldbuilding' ? <WorldbuildingWorkspace repository={repository} snapshot={snapshot} selectedStoryId={selectedStoryId} onRefresh={async () => { await refresh(); }} onNavigateDocument={(anchor) => { const scene = snapshot.scenes.find((candidate) => candidate.documentId === anchor.documentId); if (!scene) { setLocalError('This backlink source is a preserved continuous draft.'); return; } const sceneSet = snapshot.sceneSets.find((candidate) => candidate.id === scene.sceneSetId); const chapter = sceneSet && snapshot.chapters.find((candidate) => candidate.id === sceneSet.chapterId); if (chapter) { setSelectedStoryId(chapter.storyId); setSelectedChapterId(chapter.id); } setSelectedSceneId(scene.id); setMode('scene'); setWorkspaceMode('writing'); setSourceContextAnchor(anchor); setLocalError(`Source context opened: block ${anchor.blockId}`); }} onError={setLocalError} /> : <main className="workspace"><div className="workspace-head"><div><p className="eyebrow">{activeChapter?.title ?? 'Chapter'}</p><h1>{mode === 'continuous' ? 'Continuous draft' : mode === 'compose' ? 'Composed chapter' : activeScene?.title ?? 'Choose a scene'}</h1></div><div className="mode-switch"><button type="button" className={mode === 'scene' ? 'active' : ''} onClick={() => activeScene && selectScene(activeScene)}>Scenes</button><button type="button" className={mode === 'compose' ? 'active' : ''} onClick={compose}>Chapter view</button><button type="button" className={mode === 'continuous' ? 'active' : ''} onClick={openContinuous}>Continuous draft</button></div></div>
