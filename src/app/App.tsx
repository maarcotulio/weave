...
function MarkdownImportDialog({ destination, defaultTarget, canImportScene, canImportChapter, busy, onCancel, onSubmit, trigger }: { destination: MarkdownImportDestination; defaultTarget: MarkdownImportTarget; canImportScene: boolean; canImportChapter: boolean; busy: boolean; onCancel: () => void; onSubmit: (files: ImportedMarkdownFile[], target?: MarkdownImportTarget) => void; trigger?: FocusTrigger }) {
  ...
  const title = destination === 'manuscript' ? 'Import Markdown into Manuscript' : 'Import Markdown into Worldbuilding';
  ...
  const ready = files.length > 0 && (destination === 'worldbuilding' || (target === 'scene' ? canImportScene : canImportChapter));
  return <Modal eyebrow="MARKDOWN IMPORT" title={title} onClose={onCancel} closeDisabled={busy} busy={busy} trigger={trigger} descriptionId={descriptionId}>
    <p id={descriptionId}>{destination === 'manuscript' ? 'Choose one or more .md files. Each file becomes a new scene, or a new chapter with one scene containing the file content.' : 'Choose one or more .md files. Each file becomes a local Markdown note, keeping its source text and deterministic wiki links.'}</p>
    ...
    {destination === 'manuscript' && <fieldset className="markdown-import-targets"><legend>Import as</legend><label className={target === 'scene' ? 'selected' : ''}><input type="radio" name="markdown-import-target" value="scene" checked={target === 'scene'} disabled={!canImportScene} onChange={() => setTarget('scene')} /><span><strong>New scene</strong><small>{canImportScene ? 'Add the file to the selected chapter.' : 'Select a chapter first.'}</small></span></label><label className={target === 'chapter' ? 'selected' : ''}><input type="radio" name="markdown-import-target" value="chapter" checked={target === 'chapter'} disabled={!canImportChapter} onChange={() => setTarget('chapter')} /><span><strong>New chapter</strong><small>{canImportChapter ? 'Create a chapter and its first scene from each file.' : 'Select a story first.'}</small></span></label></fieldset>}
    <div className="modal-actions"><button type="button" className="text-button" disabled={busy} onClick={onCancel}>Cancel</button><button type="button" className="primary-button" disabled={busy || !ready} onClick={() => onSubmit(files, destination === 'manuscript' ? target : undefined)}>Import {files.length || ''} file{files.length === 1 ? '' : 's'}</button></div>
  </Modal>;
}

function WorldbuildingTree({ snapshot, activeKey, error, onOpen, onCreate, onImport, onRenameFolder, onDeleteFolder, onRenameNote, onDeleteNote, onRenameCanvas, onDeleteCanvas, onMove }: { snapshot: ProjectSnapshot; activeKey?: string; error?: string; onOpen: (entry: WorldbuildingEntry) => void; onCreate: () => void; onImport: () => void; onRenameFolder: (folder: WorldbuildingFolder) => void; onDeleteFolder: (folder: WorldbuildingFolder) => void; onRenameNote: (note: MarkdownNote) => void; onDeleteNote: (note: MarkdownNote) => void; onRenameCanvas: (canvas: StoryCanvas) => void; onDeleteCanvas: (canvas: StoryCanvas) => void; onMove: (entry: WorldbuildingEntry, parentId?: string) => void }) {
...
export default function App() {
  ...
  const [markdownImportDialog, setMarkdownImportDialog] = useState<{ destination: MarkdownImportDestination; defaultTarget: MarkdownImportTarget; trigger?: FocusTrigger }>();
  ...
  const openMarkdownImport = (destination: MarkdownImportDestination) => {
    const trigger = captureFocusTrigger();
    return void run(async () => {
      await autosave.flush();
      if (destination === 'worldbuilding' && !await flushActiveMarkdownNote()) return;
      setMarkdownImportDialog({ destination, defaultTarget: selectedChapterId ? 'scene' : 'chapter', trigger });
    });
  };
  const importMarkdownFiles = (files: ImportedMarkdownFile[], target?: MarkdownImportTarget) => void run(async () => {
    ...
    if (dialog.destination === 'worldbuilding') {
      ...
      if (last) await openWorldbuildingTab({ kind: 'note', id: last.id }, { noteFlushed: true });
      setToast({ message: `Imported ${notes.length} Markdown note${notes.length === 1 ? '' : 's'} into Worldbuilding.` });
      return;
    }
    await autosave.flush();
    if (target === 'scene') {
